...
export async function POST(request: Request) {
  ...
  try {
    ...
    console.log(
      `Indexed ${splitDocs.length} chunks in Pinecone for resume ${newResume.id}.`
    );
...
export async function GET(request: Request) {
  ...
  try {
    // For backward compatibility, check if this is a request for a single resume ID
    const url = new URL(request.url);
    const single = url.searchParams.get("single");

    if (single === "true") {
      // Original behavior for existing functionality
      ...
      // New behavior for profile page - return all resumes
